...
#ip link set dev $INT_underlay mtu 9000
#ip link set dev $INT_underlay up
#ip addr add $IP_underlay/$netmask dev $INT_underlay
#ethtool -U $INT_underlay rx-flow-hash udp4 sdfn #RSS config
...
#ovs-vsctl add-br ${prefix}_br0
#ovs-vsctl add-port ${prefix}_br0 $INT_underlay
#ovs-vsctl add-port ${prefix}_br0 ${prefix}_vxlan0 -- set Interface ${prefix}_vxlan0 type=vxlan options:remote_ip=$IP_remote option:key=$ID

#ip link set dev ${prefix}_br0 mtu 1500
#ip link set dev ${prefix}_br0 up
