...
if [ $# == 3 ]; then
  ...
  elif [ $app == "udp" ] && [ $id == "server" ]; then
    envoy -c $config_path/7.iperf3_server_udp.yaml
  elif [ $app == "udp" ] && [ $id == "client" ]; then
    envoy -c $config_path/8.iperf3_client_udp.yaml
  else
  ...
  if [ $app == "iperf3" ] && [ $id == "server" ]; then
    ...
    ip netns exec $namespace envoy -c $config_path/7.iperf3_server_udp.yaml
  elif [ $app == "udp" ] && [ $id == "client" ]; then
    ip netns exec $namespace envoy -c $config_path/8.iperf3_client_udp.yaml
  else
    echo "Error: no the configuration file"
  fi
fi
