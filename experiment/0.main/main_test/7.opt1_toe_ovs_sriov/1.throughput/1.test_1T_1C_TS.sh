...
if [ $1 == "server" ]; then
  ...
  ip=$IP_client
  port=$PORT_iperf_client
  thread=1
  min_traffic_size=128*1024
  max_traffic_size=16*1024*1024*1024
  namespace="LM_ns0"
  client_path="../../../../1.app/client_env"

  log_path="../../../../4.result/$scene/1.throughput/1.test_1T_1C_TS"
  if [ ! -d $log_path ]; then
    mkdir -p $log_path
  fi
  ...
  for (( i=0; i<5; i++)); do
    ...
    $client_path/1.iperf3_TS.sh $ip $port $thread $min_traffic_size $max_traffic_size $namespace | tee $log_path/$log_file
